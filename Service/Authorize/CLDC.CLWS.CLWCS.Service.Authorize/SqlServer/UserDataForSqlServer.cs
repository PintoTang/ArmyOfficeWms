...
namespace CLDC.CLWS.CLWCS.Service.Authorize.SqlServer
{
    public sealed class UserDataForSqlServer : UserDataAbstract
    {
        ...
        {
            string sql = string.Format("SELECT count(1) as Num FROM T_ST_ACCOUNT T WHERE T.ACC_CODE='{0}'", data.Account.AccCode);
