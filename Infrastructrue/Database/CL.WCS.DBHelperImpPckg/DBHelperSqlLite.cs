...
{
	public class DBHelperSqlLite : IDbHelper
	{
		...
		public DBHelperSqlLite(string ConnectionString, string strDatabaseName)
		{
		...
		public string ExecuteScalar(string strSql)
		{
			...
			try
			{
				using (SQLiteConnection conn = new SQLiteConnection(dbConnectionString))
				{
				    ...
				    using (SQLiteCommand cmd = new SQLiteCommand(strSql))
				    {
                        cmd.Connection = conn;
				...
				using (SQLiteConnection conn = new SQLiteConnection(dbConnectionString))
				{
				    ...
				    using (SQLiteCommand cmd = new SQLiteCommand(strSql, conn))
				    {
                        ...
                        if (paramList != null && paramList.Keys.Count > 0)
                        {
                            ...
                            {
                                cmd.Parameters.Add(new SQLiteParameter(pn, paramList[pn]));
                            }
                        }
                        ret = cmd.ExecuteNonQuery();
		...
		{
			int val = 0;
			using (SQLiteConnection myConnection = new SQLiteConnection(dbConnectionString))
			{
			    ...
			    using (SQLiteCommand myCommand = myConnection.CreateCommand())
			    {
			        ...
			        using (SQLiteTransaction myTrans = myConnection.BeginTransaction())
			        {
			...
			try
			{
				using (SQLiteConnection myConnection = new SQLiteConnection(dbConnectionString))
				{
					...
					foreach (string strSql in cmdText)
					{
					    ...
					    using (SQLiteDataAdapter sda = new SQLiteDataAdapter(strSql, myConnection))
					    {
		...
		public DataSet ExecuteProcedure(string strProcedureName, params string[] InputParms)
		{
			...
			try
			{
				using (SQLiteConnection myConnection = new SQLiteConnection(dbConnectionString))
				{
				    ...
				    using (SQLiteCommand cmd = new SQLiteCommand(strProcedureName, myConnection))
				    {
                        cmd.CommandTimeout = TimeOut;
		...
		public int ExecuteProcedure(string strProcedureName, params DbParameter[] parameters)
		{
			...
			try
			{
				using (SQLiteConnection myConnection = new SQLiteConnection(dbConnectionString))
				{
					...
					SQLiteCommand cmd = new SQLiteCommand(strProcedureName, myConnection);
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.CommandTimeout = TimeOut;
					if (parameters != null && parameters.Length > 0)
					{
