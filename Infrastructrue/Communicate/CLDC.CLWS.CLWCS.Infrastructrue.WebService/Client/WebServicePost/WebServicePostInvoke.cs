...
{
    public class WebServicePostInvoke : IWebNetInvoke
    {
        ...
        public OperateResult<string> ServiceRequest<T>(string url, string methodName, params object[] cmd) where T : IResponse, new()
        {
            ...
            if (CommunicationMode== CommunicationModeEnum.Production)
            {
                try
                {
                    ...
                    object reponse = WebServiceHelper.InvokeWebServicePost(url, methodName, hashtableCmd,string.Empty,TimeOut);
        ...
        public OperateResult<string> ServiceRequest<T>(string url, string methodName, string cmd, string soapAction = "", Hashtable hearder = null) where T : IResponse, new()
        {
            ...
            if (CommunicationMode == CommunicationModeEnum.Production)
            {
                try
                {
                    ...
                    object reponse = WebServiceHelper.InvokeWebServicePost(url, methodName, hashtableCmd, soapAction,TimeOut);
                    reponseResult.Content = reponse.ToString();
                    reponseResult.IsSuccess = true;
                    string logMessange = string.Format("调用：{0} 的方法：{1} 参数：{2} 成功,返回参数：{3}", url, methodName, cmd,
        ...
        public OperateResult<string> ServiceRequest<T>(string url, string methodName,Hashtable paramHashtable) where T : IResponse ,new()
        {
            ...
            if (CommunicationMode == CommunicationModeEnum.Production)
            {
                try
                {
                    object reponse = WebServiceHelper.InvokeWebServicePost(url, methodName, paramHashtable,string.Empty,TimeOut);
                    reponseResult.Content = reponse.ToString();
                    reponseResult.IsSuccess = true;
