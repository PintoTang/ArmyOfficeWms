...
{
    public class WebServicePostSoapInvoke : IWebNetInvoke
    {
        public Action<string, EnumLogLevel> NotifyMsgToUiEvent;
        private string _logDisplayName = "WebServiceSoap调用接口";
        ...
        public OperateResult<string> ServiceRequest<T>(string url, string methodName, params object[] cmd) where T : IResponse, new()
        {
            ...
            Hashtable hashtableCmd = new Hashtable();
            if (cmd != null && cmd.Length > 0)
            {
                foreach (object para in cmd)
                {
                    cmdString.Append(para.ToString());
                    hashtableCmd.Add("cmd", para);
                }
            }
            ...
            if (CommunicationMode== CommunicationModeEnum.Production)
            {
                try
                {
                    OperateResult<XmlDocument> resposeDocResult = HttpHelper.InvokeWebMethod(url, methodName, hashtableCmd);
                    reponseResult.IsSuccess = resposeDocResult.IsSuccess;
                    reponseResult.Content = resposeDocResult.Content.InnerText;
                ...
                catch (Exception ex)
                {
                    ...
                    LogMessage(logMessange, EnumLogLevel.Info, true);
                    reponseResult.Message = OperateResult.ConvertException(ex);
            ...
            if (CommunicationMode== CommunicationModeEnum.Automatic )
            {
                ...
                string msg = string.Format("[自动返回] 请求信息：Url：{0} 方法名：{1} 参数：{2} 返回结果：{3}", url, methodName, cmdString, responseResult.ToSuccessMsg());
                LogMessage(msg, EnumLogLevel.Info, true);
                reponseResult.IsSuccess = true;
                reponseResult.Content = responseResult.ToSuccessMsg();
        ...
        public OperateResult<string> ServiceRequest<T>(string url, string methodName, string cmd, string soapAction = "", Hashtable hearder = null) where T : IResponse, new()
        {
            OperateResult<string> reponseResult = new OperateResult<string>();
            if (CommunicationMode == CommunicationModeEnum.Production)
            {
                try
                {
                    ...
                    hashtableCmd.Add("cmd", cmd);
                    //object reponse = WebServiceHelper.InvokeWebServicePost(url, methodName, hashtableCmd, soapAction);
                    OperateResult<XmlDocument> resposeDocResult = HttpHelper.InvokeWebMethod(url, methodName, hashtableCmd);
                    reponseResult.IsSuccess = resposeDocResult.IsSuccess;
                    reponseResult.Content = resposeDocResult.Content.InnerText;
                ...
                {
                    string logMessange = string.Format("调用：{0} 的方法：{1} 参数：{2} 发生异常：{3}", url, methodName, cmd,
                        OperateResult.ConvertException(ex));
                    LogMessage(logMessange, EnumLogLevel.Info, true);
            ...
            {
                T responseResult = new T();
                string msg = string.Format("[自动返回] 请求信息：Url：{0} 方法名：{1} 参数：{2} 返回结果：{3}", url, methodName, cmd, responseResult.ToSuccessMsg());
                LogMessage(msg, EnumLogLevel.Info, true);
                reponseResult.IsSuccess = true;
                reponseResult.Content = responseResult.ToSuccessMsg();
                reponseResult.Message = msg;
        ...
        public OperateResult<string> ServiceRequest<T>(string url, string methodName,Hashtable paramHashtable) where T : IResponse ,new()
        {
            ...
            if (CommunicationMode == CommunicationModeEnum.Production)
            {
                try
                {
                    ...
                    //cmdString.Append(cmd);
                    //hashtableCmd.Add("cmd", cmd);
                    OperateResult<XmlDocument> resposeDocResult = HttpHelper.InvokeWebMethod(url, methodName, paramHashtable);
                    reponseResult.IsSuccess = resposeDocResult.IsSuccess;
                    reponseResult.Content = resposeDocResult.Content.InnerText;
            ...
            {
                T responseResult = new T();
                string msg = string.Format("[自动返回] 请求信息：Url：{0} 方法名：{1} 参数：{2}  返回结果：{3}", url, methodName, HashtableConvertToString(paramHashtable),responseResult.ToSuccessMsg());
                LogMessage(msg, EnumLogLevel.Info, true);
                reponseResult.IsSuccess = true;
                reponseResult.Content = responseResult.ToSuccessMsg();
                reponseResult.Message = msg;
