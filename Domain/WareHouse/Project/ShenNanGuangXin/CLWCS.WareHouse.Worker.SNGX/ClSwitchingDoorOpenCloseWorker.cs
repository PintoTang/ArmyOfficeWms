...
{
    public class ClSwitchingDoorOpenCloseWorker : SwtichingWorkerAbstract<SwitchingWorkerBusinessAbstract>
    {
        ...
        public override OperateResult SwithValueChangeHandle(object sender,int newValue)
        {
            if (newValue<=0)
            {
                LogMessage(string.Format("收到就绪：{0} ，不处理业务", newValue), EnumLogLevel.Info, true);
            ...
            if (switchDevices != null && switchDevices.Count > 0)
            {
                ...
                LogMessage(string.Format("收到就绪：{0} 触发设备业务处理", newValue), EnumLogLevel.Info, true);
            }
            else
            {
                LogMessage(string.Format("收到就绪：{0}，但是找不到任何设备", newValue), EnumLogLevel.Info, true);
            }
            return OperateResult.CreateSuccessResult();
        }
        ...
        protected override OperateResult<WareHouseViewModelBase> CreateViewModel()
        {
            ...
            viewModelResult.IsSuccess = false;
        ...
        protected override Window CreateConfigView()
        {
            ...
            WorkerConfigViewModel<ClSwitchingDoorOpenCloseWorker, SwitchingWorkerProperty> viewModel = new WorkerConfigViewModel<ClSwitchingDoorOpenCloseWorker, SwitchingWorkerProperty>(this, WorkerProperty);
