...
{
    public class ClSwitchingWorker : SwtichingWorkerAbstract<SwitchingWorkerBusinessAbstract>
    {
        ...
        public override OperateResult SwithValueChangeHandle(object sender, int newValue)
        {
            if (newValue != 888)
            {
                LogMessage(string.Format("收到就绪：{0}，非-888不做处理", newValue), EnumLogLevel.Info, true);
        ...
        protected override Window CreateConfigView()
        {
            ...
            WorkerConfigViewModel<ClSwitchingWorker, SwitchingWorkerProperty> viewModel = new WorkerConfigViewModel<ClSwitchingWorker, SwitchingWorkerProperty>(this, WorkerProperty);
