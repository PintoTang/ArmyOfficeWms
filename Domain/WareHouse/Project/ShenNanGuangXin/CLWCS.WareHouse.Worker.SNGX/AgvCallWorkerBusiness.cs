...
{
    public class AgvCallWorkerBusiness : SwitchingWorkerBusinessAbstract
    {
        ...
        {
            var switchDevice = device as SwitchDeviceAbstract;
            if (switchDevice != null)
            {
                switchDevice.DeviceControl.Communicate.Write(DataBlockNameEnum.IsRequestCompleteDataBlock, newValue==1);
            }
            if (newValue == 1)//等于1才调用wms呼叫agv搬运
            {
                ...
                AgvCallModel cmd = new AgvCallModel { SrcAddr = device.CurAddress.FullName, TaskType = 1, };
                string cmdPara = cmd.ToJson();
                string interFaceName = "NotifyAgvCarry";
                NotifyElement element = new NotifyElement("", interFaceName, "呼叫AGV搬运", null, cmdPara);
