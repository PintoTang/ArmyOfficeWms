...
{
    public class XPointStationWorkerBusiness : SwitchingWorkerBusinessAbstract
    {
        ...
        {
            string iniPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "nodeStatus.ini");
            ...
            try
            {
                ...
                string nodeStatus=IniHelper.ReadIniData("NodeStatus", wmsAddress.Content,"", iniPath);

                if (nodeStatus == newValue.ToString())
                {
                    return OperateResult.CreateSuccessResult();
                }
                CmdNodeStatusModel cmd = new CmdNodeStatusModel { CurrAddr = wmsAddress.Content, IsAlarm=newValue==1 };
                string cmdPara = cmd.ToJson();
                string interFaceName = "ReportNodeStatus";
                NotifyElement element = new NotifyElement("", interFaceName, "安全光栅/避障开关/安全门的信号上报", null, cmdPara);
                ...
                IniHelper.WriteIniData("NodeStatus", wmsAddress.Content, newValue.ToString(), iniPath);
