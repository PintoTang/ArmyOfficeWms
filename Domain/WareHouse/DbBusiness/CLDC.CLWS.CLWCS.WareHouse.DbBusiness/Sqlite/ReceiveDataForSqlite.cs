...
{
    public sealed class ReceiveDataForSqlite : ReceiveDataAbstract
    {
        public ReceiveDataForSqlite(IDbHelper dbHelper)
            : base(dbHelper)
        {
        }
        ...
        public override List<ReceiveDataModel> SelectData(ReceiveDataFilter filter)
        {
            ...
            string selectSql = string.Format(@"select * from T_BO_ReceiveData  WHERE 1=1 and {0} order by rd_receivedate limit {1} offset {2} ", where, filter.PageSize, filter.PageIndex);
