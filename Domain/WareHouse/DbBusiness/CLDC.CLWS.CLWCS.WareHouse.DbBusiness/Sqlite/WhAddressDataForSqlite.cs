...
namespace CLDC.CLWS.CLWCS.WareHouse.DbBusiness.Sqlite
{
    
    public sealed class WhAddressDataForSqlite : WhAddressDataAbstract
    {
        
        public WhAddressDataForSqlite(IDbHelper dbHelper)
            : base(dbHelper)
        {
        }
        ...
        public override List<WhAddressModel> SelectData(WhAddressDataFilter filterModel)
        {
            ...
            string selectSql = string.Format(@"select * from T_BO_WH_ADDRESS  WHERE 1=1 and {0} order by WCS_ADDR limit {1} offset {2} ", where, filterModel.PageSize, filterModel.PageIndex);
            //string selectSql = string.Format(@"SELECT TOP {0} * FROM(SELECT row_number() OVER(ORDER BY T_BO_WH_ADDRESS DESC) AS rownumber,* FROM T_BO_WH_ADDRESS WHERE 1=1 AND {2}) temp_row WHERE rownumber>({1}-1)*{0}", filterModel.PageSize, filterModel.PageIndex, where);
            try
            {
