...
namespace CLDC.CLWS.CLWCS.WareHouse.DbBusiness
{
    ...
    /// 
    /// </summary>
    public sealed class TransportMsgDataForSqlServer : TransportMsgDataAbstract
    {
        public TransportMsgDataForSqlServer(IDbHelper dbHelper)
            : base(dbHelper)
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    try
                    {
                        ...
                        if (dr["ADDDATETIME"] != null && !string.IsNullOrEmpty(dr["ADDDATETIME"].ToString()))
                        {
                            msgMode.AddDateTime = Convert.ToDateTime(dr["ADDDATETIME"]);
                        }
                        if (dr["UPDATEDATETIME"] != null && !string.IsNullOrEmpty(dr["UPDATEDATETIME"].ToString()))
                        {
