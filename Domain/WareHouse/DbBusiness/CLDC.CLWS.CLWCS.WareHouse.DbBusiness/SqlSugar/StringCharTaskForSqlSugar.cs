...
{
    public sealed class StringCharTaskForSqlSugar : StringCharTaskAbstract
    {
        public StringCharTaskForSqlSugar(IDbHelper dbHelper)
            : base(dbHelper)
        {
        }
        ...
        public override OperateResult IsExist(StringCharTaskModel data)
        {
            ...
            try
            {
                bool isExists=DbHelper.IsExist<StringCharTaskModel>(t => t.WH_CODE == WareHouseCode && t.UniqueCode == data.UniqueCode);
                //string sql =
                //    string.Format(
                //        @"SELECT COUNT(1) AS Num FROM T_AC_STRINGCHARTASK t WHERE t.TASK_CODE='{0}' AND t.WH_CODE='{1}'",
        ...
        public override OperateResult Update(StringCharTaskModel data)
        {
            ...
            try
            {
                bool result = DbHelper.Update<StringCharTaskModel>(t => new StringCharTaskModel
                {
                    ProcessStatus = data.ProcessStatus,
                    DeviceId = data.DeviceId,
                }, t => t.UniqueCode == data.UniqueCode && t.WH_CODE == WareHouseCode) > 0;
                //string sql =
                //    string.Format(
                //        @"UPDATE T_AC_STRINGCHARTASK SET TASK_PROCESS_STATUS={0},BELONG_DEVICE_ID={1} WHERE TASK_CODE='{2}' AND WH_CODE='{3}'",
                //        (int)data.ProcessStatus, data.DeviceId, data.UniqueCode, WareHouseCode);
                updateResult.IsSuccess = result;// DbHelper.ExecuteNonQuery(sql);
                return updateResult;
            }
            catch (Exception ex)
            {
        ...
        public override OperateResult Insert(StringCharTaskModel data)
        {
            ...
            try
            {
                ...
                //            string sql =
                //string.Format(
                //    @"INSERT INTO T_AC_STRINGCHARTASK(TASK_CODE,UPPER_TASK_CODE,LOWER_TASK_CODE,TASK_SOURCE,TASK_TYPE,TASK_PROCESS_STATUS,BELONG_DEVICE_ID,TASK_VALUE,WH_CODE) 
                //                                                                  VALUES ('{0}','{1}','{2}',{3},{4},{5},{6},'{7}','{8}')",
                //    data.UniqueCode, data.UpperTaskCode, data.LowerTaskCode, (int)data.TaskSource, (int)data.TaskType,
                //    (int)data.ProcessStatus, data.DeviceId, data.TaskValue, WareHouseCode);

                insertResult.IsSuccess = result;//DbHelper.ExecuteNonQuery(sql);
                return insertResult;
            }
            catch (Exception ex)
            {
        ...
        public override OperateResult<List<StringCharTaskModel>> GetUnFinishTask(int deviceId)
        {
            ...
            OperateResult<List<StringCharTaskModel>> getResult = OperateResult.CreateFailedResult(content, "初始化");
            try
            {
                content=DbHelper.QueryList<StringCharTaskModel>(t => t.DeviceId == deviceId && t.ProcessStatus == 2);
                //string sql = string.Format(@"SELECT *FROM T_AC_STRINGCHARTASK t WHERE t.BELONG_DEVICE_ID='{0}' AND t.TASK_PROCESS_STATUS='{1}'", deviceId, 2);
                ...
                //        {
                //            StringCharTaskModel task = new StringCharTaskModel
                //            {
        ...
        public override OperateResult<List<StringCharTaskModel>> GetTaskByStatus(int deviceId, List<int> statusList)
        {
            ...
            //}
            List<StringCharTaskModel> list = null;
            OperateResult <List<StringCharTaskModel>> getResult = OperateResult.CreateFailedResult(list,"初始化");
            try
            {
                list = DbHelper.QueryList<StringCharTaskModel>(t => t.DeviceId == deviceId && statusList.Contains((int)t.ProcessStatus));
                //string sql = string.Format(@"SELECT *FROM T_AC_STRINGCHARTASK t WHERE t.BELONG_DEVICE_ID='{0}' AND t.TASK_PROCESS_STATUS in ('{1}')", deviceId, statusIds);
                //using (DataSet ds = DbHelper.GetDataSet(sql))
                //{
                //    if (ds != null && ds.Tables[0].Rows.Count > 0)
                //    {
