...
{
    public class OrderDataForSqlSugar : OrderDataAbstract
    {
        public OrderDataForSqlSugar(IDbHelper dbhelper)
        {
        ...
        {
            bool isExists=DbHelper.IsExist<ExOrder>(t => t.OrderId == order.OrderId && t.WhCode == WareHouseCode);
            return isExists;
            ...
            if (null != order.PriorDeviceList && order.PriorDeviceList.Count > 0)
            {
                foreach (DeviceName device in order.PriorDeviceList)
                {
                    if (device.Type == "RGV")
                    {
                        ...
                        order.HelperRGVNo = priorRGVNo;
                    }
                }
            }

            order.WhCode = WareHouseCode;
            using(var dbContext = DbHelper.CreateContext())
            {
                dbContext.Ado.BeginTran();
                bool result=DbHelper.AddReturnBool(dbContext,order);
                if (!result)
                {
                ...
                if (SystemConfig.Instance.IsUseCellPile)
                {
                    if ((order.OrderType == OrderTypeEnum.Out || order.OrderType == OrderTypeEnum.Move) && order.StartAddr.Type == "Cell")
                    {
                        ...
                        result=dbContext.Ado.ExecuteCommand(sql2) > 0;
                        if (!result)
                        {
                            dbContext.Ado.RollbackTran();
                            return false;
                    ...
                    if ((order.OrderType == OrderTypeEnum.In || order.OrderType == OrderTypeEnum.Move) && order.DestAddr.Type == "Cell")
                    {
                        ...
                        result = dbContext.Ado.ExecuteCommand(sql3) > 0;
                        if (!result)
                        {
                            dbContext.Ado.RollbackTran();
                            return false;
        ...
        private bool UpdateOrder(ExOrder order)
        {
            ...
            string startAddrName = order.StartAddr != null ? order.StartAddr.FullName : string.Empty;
            string currAddrName = order.CurrAddr != null ? order.CurrAddr.FullName : string.Empty;
            string destAddrName = order.DestAddr != null ? order.DestAddr.FullName : string.Empty;
            string nextAddrName = order.NextAddr != null ? order.NextAddr.FullName : string.Empty;
            bool result = DbHelper.Update<ExOrder>(t => new ExOrder
            {
                PileNo=order.PileNo,
                StartAddrName = startAddrName,
                CurrAddrName= currAddrName,
                DestAddrName= destAddrName,
                OrderTypeName = order.OrderType.ToString(),
                Status=order.Status,
                HelperRGVNo= order.HelperRGVNo,
                OrderPriority=order.OrderPriority,
                DocumentCode=order.DocumentCode,
                BackFlag = order.BackFlag,
                NextAddrName= nextAddrName,
                CurHandlerId=order.CurHandlerId,
                FinishType=order.FinishType,
                SourceName=order.Source.ToString(),
                IsReport=order.IsReport,

            }, t => t.OrderId == order.OrderId && t.WhCode == WareHouseCode)>0;
            ...
            //    order.PileNo, order.StartAddr!=null?order.StartAddr.FullName:string.Empty, order.CurrAddr!=null?order.CurrAddr.FullName:string.Empty, order.DestAddr!=null?order.DestAddr.FullName:string.Empty, 
            //    order.OrderType.ToString(), (int)order.Status, priorRGVNo, order.OrderPriority, order.DocumentCode, order.BackFlag, order.NextAddr!=null?order.NextAddr.FullName:string.Empty, order.CurHandlerId, (int)order.FinishType, order.Source.ToString(), 
            //    order.IsReport, order.OrderId, WareHouseCode);
        ...
        public override List<ExOrder> GetUnfinishedOrders()
        {
            ...
            List<ExOrder> orderList = DbHelper.QueryList<ExOrder>(t => t.Status != StatusEnum.CmdSent && t.WhCode == WareHouseCode, "STATUS desc");
        ...
        {
            List<ExOrder> orderList = DbHelper.QueryList<ExOrder>(t => t.Status != StatusEnum.TransportCompleted && t.WhCode == WareHouseCode, "STATUS desc");
        ...
        public override List<ExOrder> GetUnifinishedOrders(int workerId, List<StatusEnum> statusList)
        {
            ...
            //}
            List<ExOrder> orderList = DbHelper.QueryList<ExOrder>(t => statusList.Contains(t.Status) && t.CurHandlerId== workerId && t.WhCode == WareHouseCode, "STATUS desc");
