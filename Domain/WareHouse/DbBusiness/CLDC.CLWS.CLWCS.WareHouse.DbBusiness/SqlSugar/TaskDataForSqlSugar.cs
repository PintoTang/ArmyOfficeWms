...
{
    public sealed class TaskDataForSqlSugar : TaskDataAbstract
    {
        public TaskDataForSqlSugar(IDbHelper dbHelper) : base(dbHelper)
        {
        }
        ...
        public override OperateResult IsExist(TaskModel data)
        {
            ...
            try
            {
                bool isExists = DbHelper.IsExist<TaskModel>(t => t.TaskCode == data.TaskCode && t.WhCode == WareHouseCode);
        ...
        public override OperateResult Update(TaskModel data)
        {
            ...
            try
            {
                bool result = DbHelper.Update<TaskModel>(t => new TaskModel
                {
                    TaskProcessStatus = data.TaskProcessStatus,
                }, t => t.TaskCode == data.TaskCode && t.WhCode == WareHouseCode) > 0;

                //var sql = string.Format("UPDATE T_AC_TASK SET TASK_PROCESS_STATUS={0} WHERE TASK_CODE='{1}' AND WH_CODE='{2}'", 
                //    (int)data.TaskProcessStatus, data.TaskCode, data.WhCode);
                //int result = DbHelper.ExecuteNonQuery(new[] { sql });
                if (!result)
                {
                    updateResult.IsSuccess = false;
                    updateResult.Message = string.Format("更新数据失败，更新语句");
                    return updateResult;
                }
