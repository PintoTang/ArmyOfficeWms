...
{
    public class OrderDataForOracle:OrderDataAbstract
    {
		public OrderDataForOracle(IDbHelper dbhelper)
		{
		...
		{
			string strsql = "select nvl(max(" + fieldName + "),0) from " + tableName + " " + where;
		...
		{
			string strsql = "select nvl(min(" + fieldName + "),0) from " + tableName + " " + where;
		    using (DataSet ds = DbHelper.GetDataSet(strsql))
		    {
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
