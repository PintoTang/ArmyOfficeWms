...
{
    public sealed class UpperInterfaceDataForOracle : UpperInterfaceDataAbstract
    {
        public UpperInterfaceDataForOracle(IDbHelper dbHelper)
            : base(dbHelper)
        {
        }
        ...
        public override OperateResult Update(UpperInterfaceInvoke interfaceInfo)
        {
            ...
            try
            {
                string updateSql = string.Format(@"UPDATE T_BO_UPPERINTERFACEINVOKE SET INVOKETIME='{0}',RESULT='{1}',INVOKEDELAY='{2}',MESSAGE='{3}',INVOKESTATUS='{4}',FIRSTINVOKEDATETIME=TO_DATE('{5}','SYYYY-MM-DD HH24:MI:SS'),INVOKEFINISHDATETIME=TO_DATE('{6}','SYYYY-MM-DD HH24:MI:SS') WHERE GUID_ID='{7}'",
                ...
                using (DataSet ds = DbHelper.GetDataSet(selectSql))
                {
                    if (ds != null && ds.Tables.Count > 0 && ds.Tables.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in ds.Tables[0].Rows)
                        {
                            ...
                            if (DateTime.TryParse(dr["ADDDATE"].ToString(), out addDateTime))
                            {
                                invoke.AddDateTime = addDateTime;
                            }
                            invoke.BusinessName = dr["BUSINESSNAME"].ToString();
                            invoke.CallBackFuncName = dr["CALLBACKFUNC"].ToString();
                            DateTime firstInvokDateTime;
                            if (DateTime.TryParse(dr["FIRSTINVOKEDATETIME"].ToString(), out firstInvokDateTime))
                            {
                                invoke.FirstInvokDateTime = firstInvokDateTime;
                            }
                            invoke.GuidId = dr["GUID_ID"].ToString();
                            invoke.InvokeDelay = long.Parse(dr["INVOKEDELAY"].ToString());
                            DateTime invokeFinishDateTime;
                            if (DateTime.TryParse(dr["INVOKEFINISHDATETIME"].ToString(), out invokeFinishDateTime))
                            {
                                invoke.InvokeFinishDateTime = invokeFinishDateTime;
                            }
                            invoke.Barcode = dr["BARCODE"].ToString();
                            invoke.InvokeStatus = (InvokeStatusMode)Enum.Parse(typeof(InvokeStatusMode), dr["INVOKESTATUS"].ToString());
                            invoke.InvokeTime = ConvertHepler.ConvertToInt(dr["INVOKETIME"].ToString());
