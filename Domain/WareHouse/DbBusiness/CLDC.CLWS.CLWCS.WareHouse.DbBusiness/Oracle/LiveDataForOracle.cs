...
{
    public sealed class LiveDataForOracle : LiveDataAbstract
    {
        public LiveDataForOracle(IDbHelper dbHelper)
            : base(dbHelper)
        {
        }
        public override OperateResult IsExist(LiveData data)
        {
            string sql = string.Format(" SELECT count(1) FROM T_AC_LIVESDATA T WHERE T.DEVICE_ID='{0}' AND T.WH_CODE='{1}' AND T.DATA_INDEX='{2}'", data.DeviceId, WareHouseCode, data.Index);
            string countStr = DbHelper.ExecuteScalar(sql);
            int count;
        ...
        {
            string sql = string.Format(" SELECT *FROM T_AC_LIVESDATA T WHERE T.DEVICE_ID='{0}' AND T.WH_CODE='{1}' AND T.DATAVALUE='{2}' AND T.HANDLESTATUS='{3}' ", data.DeviceId, WareHouseCode, data.DataValue, (int)data.HandleStatus);
