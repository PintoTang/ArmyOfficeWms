...
namespace CLDC.CLWS.CLWCS.WareHouse.Worker
{
    ...
    /// 杭叉AGV调度的工作逻辑
    /// </summary>
    public class MNLMChaAgvDispatchWorker : DispatchWorkerAbstract<MNLMChaWorkerBusiness>
    {

        private MNLMChaDispatchWorkerProperty _workerProperty = new MNLMChaDispatchWorkerProperty();

        public new MNLMChaDispatchWorkerProperty WorkerProperty
        {
        ...
        public OperateResult NotifyDeviceStatusHandler(int wcsAgvId, int onlineStatus, int expStatus, int expCode)
        {
            ...
            if (agvDevice == null)
            {
                string msg = string.Format("设备：{0} 不是Agv设备，请检查设备编号是否配置正确", assistant.Device.Name + assistant.Id);
                LogMessage(msg, EnumLogLevel.Error, true);
                return OperateResult.CreateFailedResult(msg, 1);
            }
        ...
        public OperateResult NotifyExeResultEventHandler(int wcsAgvId, int exeStatus, string taskNo)
        {
            ...
            AgvMoveStepEnum step = ConvertAgvMoveStep((MNLMChaAgvMoveStepEnum)exeStatus);
            int orderId = int.Parse(taskNo);
            agvDevice.HandleMoveStepChange(orderId, step);
            return OperateResult.CreateSuccessResult();
        }

        private AgvMoveStepEnum ConvertAgvMoveStep(MNLMChaAgvMoveStepEnum step)
        {
            switch (step)
            {
                case MNLMChaAgvMoveStepEnum.Executing:
                    return AgvMoveStepEnum.PickStart;
                case MNLMChaAgvMoveStepEnum.PickFinish:
                    return AgvMoveStepEnum.PickFinish;
                case MNLMChaAgvMoveStepEnum.MoveWithLoaded:
                    break;
                case MNLMChaAgvMoveStepEnum.PutFinish:
                    return AgvMoveStepEnum.PutFinish;
                case MNLMChaAgvMoveStepEnum.Finish:
                    return AgvMoveStepEnum.Finish;
                case MNLMChaAgvMoveStepEnum.Exception:
                    break;
            }
            return AgvMoveStepEnum.MoveStart;
        }
        ...
        protected override OperateResult ParticularConfig()
        {
            ...
            try
            {
                ...
                using (StringReader sr = new StringReader(workerConfigXml))
                {
                    try
                    {
                        WorkerProperty = (MNLMChaDispatchWorkerProperty)XmlSerializerHelper.DeserializeFromTextReader(sr, typeof(MNLMChaDispatchWorkerProperty));
        ...
        private OperateResult RegisterHandler()
        {
        ...
        protected override OperateResult<WareHouseViewModelBase> CreateViewModel()
        {
            ...
            MNLMChaWorkerViewModel viewModel = new MNLMChaWorkerViewModel(this);
        ...
        protected override Window CreateConfigView()
        {
            ...
            WorkerConfigViewModel<MNLMChaAgvDispatchWorker, MNLMChaDispatchWorkerProperty> viewModel = new WorkerConfigViewModel<MNLMChaAgvDispatchWorker, MNLMChaDispatchWorkerProperty>(this, WorkerProperty);
