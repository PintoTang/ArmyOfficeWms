...
using CLDC.CLWS.CLWCS.WareHouse.Worker.Workers.DispatchWorkers.FourWayVehicle.AgvApi;
...
namespace CLDC.CLWS.CLWCS.WareHouse.Worker
{
    ...
    public class FourWayVehicleAgvInfo
    {
        public int WcsId
        {
            get;
        ...
        public int FourWayVehicleId { get; set; }
    }



    public class FourWayVehicleWorkerBusiness : OrderWorkerBuinessAbstract, IFourWayVehicleApi
    {
        protected override OperateResult ParticularInitlize()
        {
            DependencyFactory.GetDependency().RegisterInstance<IFourWayVehicleApi>(this);
        ...
        private void InitilizeAgvInfo(string deviceConvert)
        {
            ...
            foreach (string key_value in keyValue)
            {
                ...
                string[] wcs_FourWayVehicle = key_value.Split('_');
                if (wcs_FourWayVehicle.Length >= 2)
                {
                    FourWayVehicleAgvInfo agvinfo = new FourWayVehicleAgvInfo
                    {
                        WcsId = int.Parse(wcs_FourWayVehicle[0]),
                        FourWayVehicleId = int.Parse(wcs_FourWayVehicle[1])
                    };
        ...
        public OperateResult NotifyDeviceStatus(string cmd)
        {
            ...
            try
            {
                ...
                int FourWayVehicleId = deviceStatus.DEVICE_NO;
                ...
                int wcsAgvId = FourWayVehicleToWcs(FourWayVehicleId);
        ...
        public OperateResult NotifyExeResult(string cmd)
        {
            ...
            try
            {
                ...
                int FourWayVehicleId = deviceStatus.carNo;
        ...
        protected List<FourWayVehicleAgvInfo> AgvInfoDic = new List<FourWayVehicleAgvInfo>();
        ...
        private int FourWayVehicleToWcs(int FourWayVehicleId)
        {
            if (AgvInfoDic.Count <= 0)
            {
                return FourWayVehicleId;
            }
            if (AgvInfoDic.Exists(a => a.FourWayVehicleId.Equals(FourWayVehicleId)))
            {
                return AgvInfoDic.Find(a => a.FourWayVehicleId.Equals(FourWayVehicleId)).WcsId;
            }
            return FourWayVehicleId;
        }


        private int WcsToFourWayVehicle(int wcsId)
        {
            if (AgvInfoDic.Count <= 0)
            {
                return wcsId;
            }
            if (AgvInfoDic.Exists(a => a.FourWayVehicleId.Equals(wcsId)))
            {
                return AgvInfoDic.Find(a => a.FourWayVehicleId.Equals(wcsId)).FourWayVehicleId;
            }
            return wcsId;
        }
        ...
        protected OperateResult NotifyExeResultEventHandler(string cmd)
        {
            ...
            try
            {
                ...
                int FourWayVehicleId = exeResult.carNo;
