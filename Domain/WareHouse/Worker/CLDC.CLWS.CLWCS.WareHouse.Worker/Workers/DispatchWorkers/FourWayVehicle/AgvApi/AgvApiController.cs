...
namespace CLDC.CLWS.CLWCS.WareHouse.Worker.Workers.DispatchWorkers.FourWayVehicle.AgvApi
{
    ...
    public class AgvApiController : ApiController
    {
        ...
        {
            IFourWayVehicleApi FourWayVehicleApi = DependencyHelper.GetService<IFourWayVehicleApi>();
            ResponseResult respones=new ResponseResult(1,"失败");
            if (FourWayVehicleApi != null)
            {
                OperateResult result = FourWayVehicleApi.NotifyDeviceStatus(cmd);
                if (!result.IsSuccess)
                {
                    respones.RESULT = 0;
        ...
        public string NotifyExeResult(string cmd)
        {
            ...
            if (FourWayVehicleApi != null)
            {
                OperateResult result = FourWayVehicleApi.NotifyExeResult(cmd);
                if (!result.IsSuccess)
                {
                    respones.RESULT = 0;
                    respones.MESSAGE = result.Message;
