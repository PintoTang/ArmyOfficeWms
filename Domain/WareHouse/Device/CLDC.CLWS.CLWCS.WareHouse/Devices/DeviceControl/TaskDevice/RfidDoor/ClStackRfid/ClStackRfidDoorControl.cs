...
namespace CLDC.CLWS.CLWCS.WareHouse.Device
{
    ...
    public sealed class ClStackRfidDoorControl : StackRfidControlAbstract
    {
        ...
        public override  OperateResult<string> SendCmd<TResponse>(WebApiInvokeCmd webApiCmd)
        {
            ...
            //ResponseResult sucessResult = new ResponseResult();
            //sucessResult.IsSuccess = true;
            //result.Content = sucessResult.ToJson();
            //result.IsSuccess = true;
            //LogMessage(string.Format("下发任务：{0}",webApiCmd.InvokeCmd),EnumLogLevel.Info,true);
            //return result;
            ...
            try
            {
                ...
                hashtable.Add("cmd", webApiCmd.InvokeCmd);

                OperateResult<string> invokeResult = WebNetInvoke.ServiceRequest<TResponse>(Http, webApiCmd.MethodName, hashtable);
