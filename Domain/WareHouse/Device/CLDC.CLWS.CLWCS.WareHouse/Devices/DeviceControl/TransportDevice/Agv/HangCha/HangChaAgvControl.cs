...
namespace CLDC.CLWS.CLWCS.WareHouse.Device
{
    ...
    public class HangChaResponse
    {
        ...
        public static explicit operator HangChaResponse(string json)
        {
            return JsonConvert.DeserializeObject<HangChaResponse>(json);
        }

    }

    public class HangChaAgvControl : OrderDeviceControlAbstract
    {
        ...
        public override OperateResult DoJob(TransportMessage transMsg)
        {
            ...
            try
            {
                ...
                task.locationFrom =HangChaAgvCarWalkingPathManage.Instance.GetPostionNumAddrByConvertAddrData(transMsg.StartAddr.FullName);
                task.locationTo = HangChaAgvCarWalkingPathManage.Instance.GetPostionNumAddrByConvertAddrData(transMsg.DestAddr.FullName);
                ...
                OperateResult<string> response = WebNetInvoke.ServiceRequest<SyncResReMsgForHangChaAgv>(Http, "addTask", cmd);
                ...
                try
                {
                    HangChaResponse hangChaRespone = (HangChaResponse)message;
                    if (hangChaRespone.code.Equals(0))
                    {
                        return OperateResult.CreateSuccessResult();
                    }
                    return OperateResult.CreateFailedResult(string.Format("杭叉系统返回失败，失败原因：\r\n {0}", hangChaRespone.msg), 1);
                }
                catch (Exception ex)
                {
