...
{
    public class IdentityDeviceViewModel : DeviceViewModelAbstract<IdentifyDevice>
    {
        public IdentityDeviceViewModel(IdentifyDevice device)
            : base(device)
        {
            _controlCommunicate = (IdentifyDeviceForOpc)Device.DeviceControl.Communicate;
        ...
        private readonly IdentifyDeviceForOpc _controlCommunicate;
        ...
        private void DeleteLiveData(object obj)
        {
            ...
            {
                string recordMsg = string.Format("删除实时数据：{0} 操作：{1}",deleteData.DataValue,"成功");
                OperateLogHelper.Record(recordMsg);
                SnackbarQueue.MessageQueue.Enqueue("删除成功");
            }
            else
            {
                string recordMsg = string.Format("删除实时数据：{0} 操作：{1}", deleteData.DataValue, "失败");
                OperateLogHelper.Record(recordMsg);
