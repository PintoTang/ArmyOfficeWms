...
using CLDC.CLWS.CLWCS.WareHouse.Device.Devices.DeviceControl.TaskDevice.RfidDoor.ClStackRfid.View;
...
namespace CLDC.CLWS.CLWCS.WareHouse.Device.Devices.Device.TaskDevice.RfidDoor.ClStackRfid.ViewModel
{
    public class ClRfidDoorViewModel : DeviceViewModelAbstract<StackRfidDeviceAbstract>
    {
        ...
        private readonly StackRfidControlAbstract _deviceControl;
        public ClRfidDoorViewModel(StackRfidDeviceAbstract device)
            : base(device)
        {
            Device = device;
            _deviceControl = (StackRfidControlAbstract)Device.DeviceControl;
            InitWebApiViewModel();
            InitTaskViewModel();
        }
        ...
        public Dictionary<string, string> DicApiNameList
        {
            get
            {
                ...
                {
                    foreach (var value in Enum.GetValues(typeof(ClStackRfidDoorApiEnum)))
                    {
                        ClStackRfidDoorApiEnum em = (ClStackRfidDoorApiEnum)value;
        ...
        private void InitMethodAndCmd()
        {
            if (Application.Current != null && Application.Current.Dispatcher != null)
                ...
                {
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.SendScanTask.ToString(), new SendScanTaskCmdView(new SendScanTaskCmd { DEVICE_NO = Device.ExposedUnId }));
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.SendShow.ToString(), new SendShowCmdView(new SendShowCmd { DEVICE_NO = Device.ExposedUnId, LEVEL = MessageLevelTypeEnum.Nomal, MESSAGE = "测试", VOICE = "语音测试" }));
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.SendStackTask.ToString(), new SendStackTaskCmdView(new SendStackTaskCmd { DEVICE_NO = Device.ExposedUnId, TASK_TYPE = StackActionTypeEnum.Stack, BOX_COUNT = 15, BOX_TYPE = BoxTypeEnum.SingleMeter, SEP_COUNT = 3 }));
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.SendSwitchTask.ToString(), new SendSwitchTaskView(new SendSwitchTaskCmd { DEVICE_NO = Device.ExposedUnId, TASK_TYPE = DoorActionTypeEnum.CloseDoor }));
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.AskStackAction.ToString(),new AskStackActionView(new AskStackActionCmd(){DEVICE_NO = Device.ExposedUnId}));
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.AskStackLoadedStatus.ToString(),new AskStackLoadedStatusView(new AskStackLoadedStatusCmd(){DEVICE_NO = Device.ExposedUnId}));
                    _dicMethodCmd.Add(ClStackRfidDoorApiEnum.SendStackDispatch.ToString(), new SendStackDispatchCmdView(new SendStackDispatchCmd() { DEVICE_NO = Device.ExposedUnId }));
