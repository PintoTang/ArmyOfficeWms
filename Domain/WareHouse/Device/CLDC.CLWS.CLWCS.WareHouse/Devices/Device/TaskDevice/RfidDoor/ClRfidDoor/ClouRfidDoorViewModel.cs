...
{
    public class ClouRfidDoorViewModel : DeviceViewModelAbstract<RfidDoorAbstract>
    {
        ...
        private readonly ClStackRfidDoorControl _deviceControl;
        public ClouRfidDoorViewModel(RfidDoorAbstract device) : base(device)
        {
            Device = device;
            _deviceControl = (ClStackRfidDoorControl)Device.DeviceControl;
            InitWebApiViewModel();
            InitTaskViewModel();
        }
        ...
        public Dictionary<string, string> DicApiNameList
        {
            get
            {
                ...
                {
                    foreach (var value in Enum.GetValues(typeof(ClouRfidDoorApiEnum)))
                    {
                        ClouRfidDoorApiEnum em = (ClouRfidDoorApiEnum)value;
        ...
        private void InitMethodAndCmd()
        {
            if (Application.Current != null && Application.Current.Dispatcher != null)
                ...
                {
                    _dicMethodCmd.Add(ClouRfidDoorApiEnum.SendScanTask.ToString(), new SendScanTaskCmdView(new SendScanTaskCmd { DEVICE_NO = Device.ExposedUnId }));
                    _dicMethodCmd.Add(ClouRfidDoorApiEnum.SendShow.ToString(), new SendShowCmdView(new SendShowCmd { DEVICE_NO = Device.ExposedUnId, LEVEL = MessageLevelTypeEnum.Nomal, MESSAGE = "测试", VOICE = "语音测试" }));
                    _dicMethodCmd.Add(ClouRfidDoorApiEnum.SendSwitchTask.ToString(), new SendSwitchTaskView(new SendSwitchTaskCmd { DEVICE_NO = Device.ExposedUnId, TASK_TYPE = DoorActionTypeEnum.CloseDoor }));
                });
        }
