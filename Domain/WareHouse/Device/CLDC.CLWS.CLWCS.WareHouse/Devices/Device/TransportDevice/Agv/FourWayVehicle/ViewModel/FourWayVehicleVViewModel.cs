...
{
    public class FourWayVehicleViewModel : TransportDeviceViewModel
    {
        private readonly IWebNetInvoke _webNetInvoke;
        public FourWayVehicleViewModel(AgvDeviceAbstract device)
            : base(device)
        {
            Device = device;
            _deviceControl = (FourWayVehicleControl) Device.DeviceControl;
        ...
        private void InitMethodAndCmd()
        {
            ...
            if (Application.Current != null && Application.Current.Dispatcher != null)
                ...
                {
                    _dicMethodCmd.Add(FourWayVehicleApiEnum.addTask.ToString(), new SendAddTaskCmdView(DemoSendAddTaskCmd));
                    _dicMethodCmd.Add(FourWayVehicleApiEnum.taskState.ToString(), new SendTaskStateCmdView(DemoSendTaskStateCmd));
                    _dicMethodCmd.Add(FourWayVehicleApiEnum.carInfo.ToString(), new SendTaskStateCmdView(DemoSendTaskStateCmd));
                    _dicMethodCmd.Add(FourWayVehicleApiEnum.deleteTask.ToString(), new SendTaskStateCmdView(DemoSendTaskStateCmd));
                    _dicMethodCmd.Add(FourWayVehicleApiEnum.carOperationControl.ToString(), new SendCarOperationCmdView(DemoSendCarOperationCmd));

                });

        }

        private readonly FourWayVehicleControl _deviceControl;
        ...
        public Dictionary<string, string> DicApiNameList
        {
            get
            {
                ...
                {
                    foreach (var value in Enum.GetValues(typeof(FourWayVehicleApiEnum)))
                    {
                       FourWayVehicleApiEnum em = (FourWayVehicleApiEnum)value;
        ...
        public OperateResult<string> ExecInvokeMethod(WebApiInvokeCmd cmd)
        {
            ...
            {
                //组Json
                opResult = _webNetInvoke.ServiceRequest<SyncResReMsgForFourWayVehicle>(cmd.HttpUrl, cmd.MethodName, cmd.InvokeCmd);
            }
            catch (Exception ex)
            {
