...
{
    public class HangChaAgvRcsViewModel : TransportDeviceViewModel
    {
        private readonly HangChaAgvRcsControl _deviceControl;
        private readonly IWebNetInvoke _webNetInvoke;
        public HangChaAgvRcsViewModel(TransportDeviceBaseAbstract device)
            : base(device)
        {
            Device = device;
            _deviceControl = (HangChaAgvRcsControl)Device.DeviceControl;
        ...
        private void InitMethodAndCmd()
        {
            ...
            if (Application.Current != null && Application.Current.Dispatcher != null)
                ...
                {
                    _dicMethodCmd.Add(HangChaAgvRcsApiEnum.DeleteTask.ToString(), new DeleteTaskView(new DeleteTaskCmd()));
                    _dicMethodCmd.Add(HangChaAgvRcsApiEnum.SelectTaskInfo.ToString(), new SelectTaskInfoView(new SelectTaskInfoCmd()));
                    _dicMethodCmd.Add(HangChaAgvRcsApiEnum.SendDeviceAction.ToString(), new SendDeviceActionView(new SendDeviceActionCmd { ACTION = ActionTypeEnum.CloseDoor }));
                    _dicMethodCmd.Add(HangChaAgvRcsApiEnum.SendNextAddrPermit.ToString(), new SendNextAddrPermitView(new SendNextAddrPermitCmd()));
                    _dicMethodCmd.Add(HangChaAgvRcsApiEnum.SendTask.ToString(), new SendTaskView(new SendTaskCmd { ADDR_REQUEST = RequestTypeEnum.Request }));
                });
        ...
        public Dictionary<string, string> DicApiNameList
        {
            get
            {
                ...
                {
                    foreach (var value in Enum.GetValues(typeof(HangChaAgvRcsApiEnum)))
                    {
                        HangChaAgvRcsApiEnum em = (HangChaAgvRcsApiEnum)value;
