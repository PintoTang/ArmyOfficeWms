...
namespace CL.WCS.WPF
{
    ...
    public partial class WorkerShowView : UserControl, IMainUseCtrl
    {
        ...
        private readonly List<MenuItem> _mainMenuItem = new List<MenuItem>();
        ...
        /// tablNodeInfo  面板整体切换
        /// </summary>
        /// <param name="index">当前table 选择的位置</param>
        private void selectTablNodeInfo(string tabName)
        {
            var ucObj = WppnlWorker.Children.GetEnumerator();

            while (ucObj.MoveNext())
            {
                var nvcItem = ucObj.Current;
                Type tp = nvcItem.GetType();

                object o = tp.GetField("tabCNodeInfo", System.Reflection.BindingFlags.NonPublic
                                    | BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase).GetValue(nvcItem);

                TabControl tpTabControl = (TabControl)o;
                foreach (var item in tpTabControl.Items)
                {
                    TabItem tabItem = item as TabItem;
                    if (tabItem.Header.ToString().Contains(tabName))
                    {
                        tpTabControl.SelectedItem = tabItem;
                        break;
                    }
                }
            }
        ...
        private void DataSearchMehod()
        {
            var Loads = this.Dispatcher.BeginInvoke(new Action(() =>
           {
               ...
               foreach (var nvcItem in WppnlWorker.Children)
               {
                   UserControl userControl = (UserControl)nvcItem;
                   WareHouseViewModelBase tempViewModelBase = userControl.DataContext as WareHouseViewModelBase;
                   if (isUsedIdSearch)
                   {
                       //ID查询
                       int id = int.Parse(strInputData);
                       if (id == tempViewModelBase.Id)
                       {
                           UserControlScrol(userControl);
                   ...
                   {
                       //工位名称查询
                       if (Name == tempViewModelBase.Name)
                       {
                           UserControlScrol(userControl);
                           break;//只查询一个
                       }
